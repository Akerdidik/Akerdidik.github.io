...
We can investigate `old_rs` by firstly using `checkse`c to reveal security applied to this binary, and to check the version of libc provided we can see the `ld-2.39.so` file giving us the according version, or we can check it by: `strings libc.so.6 | grep GLIBC` and identify it as GLIBC 2.39
...
Binary print the menu containing options:
```
  1. Issue New Ticket
