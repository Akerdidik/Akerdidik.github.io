...
### Exploitation Part 1: Gain Arbitrary Write and Arbitrary Read primitives via Safe Unlink
...
### Explotiation Part 2: Leak libc base and stack then gain RCE via ROP chain into stack
...
### Exploitation: Race to Root via struct cred spray

We need to abuse the Race to our profit, first is we need to choose a target to point to, this will be our victim object for later abuse UAF. In my solution i have used `task_struct`'s `cred_struct` which is basically the 
...
- [TOCTOU in details](https://google.com)
